...
def plotly_bar_cost(
    ...
    if threshold_cost_percent:
        cost_df_sorted = filter_dataframe_by_cost_threshold(
            dataframe=cost_df, threshold_percent=threshold_cost_percent
        )
    ...
    task_order = cost_df_sorted.groupby('task_name')['cost'].sum().sort_values(
...
def filter_dataframe_by_cost_threshold(
        dataframe, threshold_percent, task_column='task_name', cost_column='cost'
) -> pd.DataFrame:
    """
    ...
    :param task_column: The task column name
    :param cost_column: The cost column name
    ...
    grouped_and_sorted_df = dataframe.groupby(task_column)[
        cost_column].sum().sort_values(ascending=False)

    total_cost = dataframe[cost_column].sum()
    cost_threshold = total_cost * (threshold_percent / 100)

    filtered_dataframe = pd.DataFrame()
    for task, cost in grouped_and_sorted_df.items():  # Iterate over the task names starting from the most expensive
        # Filter the dataframe for the current task
        task_df = dataframe[dataframe[task_column] == task]

        # If the filtered_dataframe is empty, this is the first iteration
        # So; we add the current task's dataframe to the filtered_dataframe
        if filtered_dataframe.empty:
            filtered_dataframe = pd.concat([filtered_dataframe, task_df])
        else:
            # If the filtered_dataframe is not empty, we check if the total cost of
            # the current filtered_dataframe and the current task's dataframe is less
            # than or equal to the cost threshold If it is, we add the current task's
            # dataframe to the filtered_dataframe
            if pd.concat([filtered_dataframe, task_df])[
                cost_column].sum() <= cost_threshold:
                filtered_dataframe = pd.concat([filtered_dataframe, task_df])
            else:
                # If the total cost is greater than the cost threshold, we break the
                #  loop; this means we stop adding more tasks to the filtered_dataframe
                break
